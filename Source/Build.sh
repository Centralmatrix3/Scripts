...
if [[ "$repo_name" == "Scripts" ]]; then
    ...
    for out_file in "${!ruleA_download[@]}"; do
        mapfile -t urls < <(echo "${ruleA_download[$out_file]}" | sed -E 's/^[[:space:]]+//; /^[[:space:]]*$/d')
    ...
    skip_rule=("ChinaASN" "GEOIPCN") # 排除不需要加载的规则
    should_skip_rule() {
        local platform="$1" rule="$2"; [[ "$platform" != "Sing-box" ]] && return 1
        for r in "${skip_rule[@]}"; do [[ "$rule" == "$r" ]] && return 0; done; return 1
    }
