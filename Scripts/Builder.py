...
def scripts_repo(execute_action, mode, repository):
...
def matrix_io_repo(execute_action, mode, repository):
    ...
    for rule, file in rule_local_source.items():
        for platform, extension in formats.items():
            ...
            output_file = f"{platform}/Ruleset/{rule}.{extension}"
            source_link = [f"{RULE_SOURCE_LINK}/{file}" for file in file.split()]
            source_file = [f"Scripts/Ruleset/{file}" for file in file.split()]
            if mode == "download":
                execute_action(output_file, source_link)
            else:
                execute_action(output_file, source_file)
...
def process_source(mode):
    ...
    if repository == "Scripts":
        scripts_repo(execute_action, mode, repository)
...
def process_order(lines, unknown_rule=False):
    ...
    for line in sorted(lines, key=rule_sort):
        ...
        rule_type = line.partition(",")[0].upper()
...
def process_file(file, tool, args):
    print(f"Platform: {args.platform}")
    print(f"Processed {len(file)} file(s) in: {args.file_path}")
    for file_path in file:
        try:
            if args.type or args.order:
                tool(file_path, enable_type=args.type, enable_order=args.order)
            else:
                tool(file_path)
...
def run_content_mode(args):
    if args.platform or args.file_path or args.type or args.order:
        sys.exit("Error: --download和--copy不能与转换模式使用")
    print("============== Build.py ==============")
