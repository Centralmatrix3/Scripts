/*
 * @name = Sub-Store重命名
 * @desc = Sub-Store重命名操作脚本
 * @author = keywos
 * ------------------------
 * 以下是脚本支持的参数。必须以"#"为开头多个参数使用"&"连接。禁用缓存url#noCache
 * ------------------------
 * 主要参数
 * - [in=]       自动判断机场节点名类型。优先级zh(中文) -> flag(国旗) -> quan(英文全称) -> en(英文简写)
 * * 如果不准的情况可以加参数指定:
 * - [nm]        保留没有匹配到的节点。
 * - [in=zh]     或in=cn识别中文。
 * - [in=en]     或in=us识别英文缩写。
 * - [in=flag]   或in=gq识别国旗。如果加参数in=flag则识别国旗。脚本操作前面不要添加国旗操作否则移除国旗后面脚本识别不到
 * - [in=quan]   识别英文全称。
 * - [out=]      输出节点名可选参数: (cn或zh, us或en, gq或flag, quan) 对应: (中文, 英文缩写, 国旗, 英文全称) 默认中文例如: [out=en]或out=us输出英文缩写
 *
 * 分隔符参数
 ...
 *
 * 保留参数
 * - [blkey=iplc+gpt+NF+IPLC] 用+号添加多个关键词。保留节点名的自定义字段需要区分大小写
 * * 如果需要修改保留的关键词替换成别的可用">"分割。例如: [#blkey=GPT>新名字+其他关键词]这将把"GPT"替换成"新名字"
 * * 例如: https://raw.githubusercontent.com/Keywos/rule/main/rename.js#flag&blkey=GPT>新名字+NF
 * - [blgd]      保留: 家宽 IPLCˣ²等
 * - [bl]        正则匹配保留: [0.1x, x0.2, 6x ,3x]等标识
 * - [nx]        保留1倍率与不显示倍率的。
 * - [blnx]      只保留高倍率。
 * - [clear]     清理乱名。
 * - [blpx]      如果用了上面的bl参数对保留标识后的名称分组排序。如果没用上面的bl参数单独使用blpx则不起任何作用
 * - [blockquic] blockquic=on阻止。blockquic=off不阻止
 * ------------------------
*/
