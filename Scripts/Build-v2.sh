...
download_copy() {
    ...
    : > "$output"
    for source in "$@"; do
        "$action" "$source" >> "$output" || { echo "Process Failed: $source"; exit 1; }
        echo "Processed: $source -> $output"
        echo >> "$output"
    done
}
_download_action() { curl -fsSL "$1"; }
_copy_action() { cat "$1"; }
download() { download_copy "$1" _download_action "${@:2}"; }
copy() { download_copy "$1" _copy_action "${@:2}"; }
