 ...
 * 本项目中的资源内容，主要是用于学习以及研究。不能保证本项目中的资源内容的合法性、准确性、完整性以及有效性。

 * 如果任何个人或组织认为该项目中的内容可能涉嫌侵犯其权利，则应该及时通知并提供身份证明以及所有权证明。本人将会在收到认证的文件以后删除相关的内容。

 * 本项目只收集和整理开源项目的重写和规则以及脚本。在该基础上进行修改满足我的需求作为自用库使用，本项目的所有贡献者并不负责维护重写和规则以及脚本。

 * 本项目所有重写和规则以及脚本数据均来自开源项目。不保证其资源数据的可用性。不生产数据只是开源数据的搬运工。特别感谢开源项目贡献者的默默辛勤付出。
